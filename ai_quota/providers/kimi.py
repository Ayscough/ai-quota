...
def parse_kimi(payload: dict) -> dict:
    ...
    if data.get("available_balance") is not None:
        result = {"status": "ok", "kind": "account_balance", "balance": float(data["available_balance"]),
                  "currency": "USD", "available_balance": float(data["available_balance"]),
...
class KimiProvider:
    ...
    def __init__(self, api_key=None, base_url=None):
        ...
        self.base_url = (base_url or os.getenv("KIMI_API_URL") or ("https://api.kimi.com" if self.coding else "https://api.moonshot.ai/v1")).rstrip("/")
    def fetch(self, timeout=8.0):
        ...
        return parse_kimi(get_json(f"{self.base_url}{endpoint}", {"Authorization": f"Bearer {self.api_key}"}, timeout))
