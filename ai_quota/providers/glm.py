...
def parse_glm(payload: dict) -> dict:
    ...
    if not result["windows"]:
        raise RuntimeError("GLM Coding Plan quota information missing")
    return result
...
class GLMProvider:
    ...
    def fetch(self, timeout=8.0):
        ...
        endpoint = "/monitor/usage/quota/limit"
        return parse_glm(get_json(f"{self.base_url}{endpoint}", {"Authorization": self.api_key}, timeout))
